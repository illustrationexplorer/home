...
DA_Nav.prototype._topEventBinding = function () {
    ...
    $(window).scroll(function(){
        ...
        if(parseInt(Math.round(window.scrollY)) >= parseInt(thisDA_Nav._sticky_top.offsetTop)) {
...
StickyTop.prototype.appendToDisplay = function(container, top) {
    ...
    this._sticky_top.style.cssText = ` position: fixed; width: ${width}px; z-index: 500; top: ${top}px; opacity: ${this._shown};`;
...
StickyTop.prototype._eventBinding = function(container) {
    ...
    window.onscroll = function() {
        ...
        if(current_DA_Nav === undefined || current_DA_Nav._getDisplayDeck() === undefined) {
            return false;
        }

        let current_display_deck = current_DA_Nav._getDisplayDeck();
        if((parseInt(current_display_deck.getBoundingClientRect()["top"]) > parseInt(displayToTop))  && (parseInt(current_display_deck.getBoundingClientRect()["bottom"]) < (0.67*displayVisibleHeight) + parseInt(displayToTop))){
            if(thisStickyTop._shown === 1)
                thisStickyTop._shown = 0;
                thisStickyTop.fadeTo("normal", thisStickyTop._shown);
        } else {
