    ...
    $.getJSON(DA_doc, function (json) {
        $.each(json, function (i, item){
            ...
            let display_top = $("<div></div>")
                                // .addClass("sticky-top")
                                // .css({
                                //     "top": document.querySelector("#card-display").offsetTop + 22,
...
function displaySpy() {
    ...
    $("#card-display").children(":not(.search-fail)").each(function(i, item){
        ...
        if($("." + $(item).attr("id")).is(":not(.active)") && (currentPosition < 0.5*screenH) && (($(item).height() + currentPosition) >= 0.5*screenH)) {
            ...
            // display-reminder info preloading
            $(".display-info").empty();
            $(".display-info").append($(item).find(".display-title").get(0)).append($(item).find(".display-sub-label"));
        }
    });
}
