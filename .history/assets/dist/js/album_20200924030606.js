    ...
    $.getJSON(DA_doc, function (json) {
        $.each(json, function (i, item){
            ...
            let display_top = $("<div></div>")
                                ...
                                .append($("<p></p>").addClass("display-desc active-sticky").text(DA_desc))
                                .append(sub_label);
...
function reminderSpy() {
    ...
    let current_active_sticky =document.querySelector(".deck-reminder.actively");
    ...
    allReminders.some(function(sticky, index, nodeList) {
        ...
        if(sticky.classList.contains("actively")) {
            console.log("self");
            return false;
        }

        if(current_active_sticky && (reminderToHeader > 15) && (reminderToHeader < (displayHeight / 2))) {
            console.log("A")
            current_active_sticky.classList.remove("actively");
            $($(current_active_sticky).find(".display-desc").get(0)).slideDown();
            return true;
        }

        if((reminderToHeader > -20) && (reminderToHeader <= 20)) {
            console.log("B")
            $($(sticky).find(".display-desc").get(0)).hide("fast");
            sticky.classList.add("actively");
            return true;
        }
    });
}
