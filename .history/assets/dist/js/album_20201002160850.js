...
DA_Nav.prototype._topEventBinding = function () {
    ...
    $(window).scroll(function(){
        ...
        if(thisDA_Nav._inSticky() && parseInt(Math.round(window.scrollY)) < parseInt(thisDA_Nav._display_desc.offsetTop) && parseInt(Math.round(window.scrollY)) > parseInt(thisDA_Nav._sticky_top.offsetTop) - 0.33*displayVisibleHeight) {
            thisDA_Nav._stickyToggle(true, () => thisDA_Nav._sticky_top_object.fadeTo("normal", 0));
            console.log("ininin", thisDA_Nav._DA_nav_tag);
        } else if(!thisDA_Nav._inSticky()) {
            thisDA_Nav._stickyToggle(false, () => thisDA_Nav._sticky_top_object.fadeTo("normal", 1));
