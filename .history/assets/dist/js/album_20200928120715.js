...
DP_Tab.prototype._DP_abr_list = function() {
    ...
    DP_sub_tags.forEach(tag => {
        ...
        let tag_abr = tag.substr(0, 2) + (tag.split(" ")).length + tag.substr(tag.length-2);
        DP_abr.push(tag_abr);
    });
...
DP_Tab.prototype._interactionInit = function() {
    if(this._created) {
        ...
        let DP_sub_ul = document.querySelectorAll(".btn-secondary-group > .btn-sub-list")[this._DP_id-1];
        this._DP_sub_li = DP_sub_ul.querySelectorAll("li"); // NodeList object
...
DP_Tab.prototype._eventBinding = function() {
    ...
    let thisDPTag = this;
    let DP_abr_list = this._DP_abr_list();
    ...
    thisDPTag._DP_sub_li.forEach(li => {
        ...
        li.addEventListener("click", function() {
            if(this.classList.contains("active")) {
                ...
                let targetCards = document.querySelectorAll(`.${this_DP_abr}:not(.screened-out)`);
                targetCards.forEach(node => node.classList.add("screened-out"));
                $(targetCards).fadeTo(400, 0).hide(1);
            } else {
                this.classList.toggle("active", true);
                let targetCards = document.querySelectorAll(`.${this_DP_abr}.screened-out`);
                targetCards.forEach(node => node.classList.remove("screened-out"));
                $(targetCards).show(1).fadeTo(500, 1);
            }
        });
    });

    // bind hide/visible event to primary buttons
    let this_joint_DP_abr = DP_abr_list.map(DP_abr => "#" + DP_abr).join(",");
    thisDPTag._DP_primary_btn.addEventListener("click", function () {
        console.log(this_joint_DP_abr)
        if(this.classList.contains("active")){
            this.classList.toggle("active", false);
            document.querySelectorAll(this_joint_DP_abr).forEach(li => li.classList.toggle("active", false));
        } else {
            this.classList.toggle("active", true);
            document.querySelectorAll(this_joint_DP_abr).forEach(li => li.classList.toggle("active", true));
        }
    })
