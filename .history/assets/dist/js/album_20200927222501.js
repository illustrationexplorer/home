...
function createDP(DP_doc, classStr) {
    ...
    $.getJSON(DP_doc, function(json) {
        ...
        $.each(json, function(i, item) {
            ...
            let { DP_primary_button, DP_sub_ul } = DP_Tab(item)._drawDP(btnClassStr);

            DP_Group.append(DP_primary_button).append(DP_sub_ul);
        });
...
function DP_Tab(DP_object) {
    ...
    this._DP_sub_tags = DP_sub_tag; // array
}
...
DP_Tab.prototype._drawDP = function(btnClassStr) {
    ...
    let DP_primary_button = $("<button></button>")
    ...
    return { DP_primary_button, DP_sub_ul };
}
