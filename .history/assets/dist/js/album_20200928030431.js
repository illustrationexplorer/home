    ...
    thisDPTag._DP_sub_li.forEach(li => {
        ...
        li.addEventListener("click", function() {
            if(this.classList.contains("active")) {
                ...
                document.querySelectorAll(`.${this_DP_abr}`).forEach(node => node.classList.add("hidden"));
                $("#card-display").find(`.${this_DP_abr}:not(.hidden)`).fadeTo(400, 0).hide(1);
            } else {
                this.classList.toggle("active", true);
                document.querySelectorAll(`.${this_DP_abr}`).forEach(node => node.classList.remove("hidden"));
                $("#card-display").find(`.${this_DP_abr}.hidden`).show(1).fadeTo(500, 1);
            }
        });
    });
}
