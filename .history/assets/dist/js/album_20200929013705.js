...
function DA_Nav(DA_object) {
    ...
    // color method
    this.set_color = DA_Nav._color_hash.set_color;
    this.get_color = DA_Nav._color_hash.get_color;
    
    this._created = 0; // nothing created: -1; button created: 1; display top created: 2
    ...
    this.set_color(this._DA_id, this._DA_nav_color);
    // this._DA_sub_arr.forEach((eg_object, index, arr) => {
    //     let DA_class_id = eg_object["DA_class_id"] || (500 + index + 1);
    //     let DA_class_color = eg_object["DA_class_color"] || "#EBEDF6";
    //     this.set_color(DA_class_id, DA_class_color);
    // });
}
...
DA_Nav._color_hash = {
    ...
    set_color: function(key_id, color_str) {
        key_id = key_id || 500;
        color_str = color_str || "";
        let _data = DA_Nav._color_hash._data;
        let _set_method = (function(key_id, color_str) {
            if(color_str) {
    ...
    get_color: function(key_id) {
        key_id = key_id || 500;
        let _data = DA_Nav._color_hash._data;
        let _get_method = (function(key_id) {
            if(_data.hasOwnProperty(key_id)) 
                return _data[key_id];
            else
...
DA_Nav.prototype.getJointTag = function() {
    return this._DA_nav_tag.split(" ").join("_");
}

DA_Nav.prototype.drawDANav= function() {
    let classString = "btn btn-block text-left";
    let DA_joint_tag = this.getJointTag();

    let DA_nav_btn = $("<a></a>").addClass([classString, DA_joint_tag].join(" "))
...
DA_Nav.prototype.drawDATop = function() {
    ...
    thisDA_Nav._DA_sub_arr.forEach(eg_object => {
        ...
        let DA_class_id = eg_object["DA_class_id"] || (500 + index + 1);
        let DA_class_color = eg_object["DA_class_color"] || "#EBEDF6";
        thisDA_Nav.set_color(DA_class_id, DA_class_color);
...
function AIE_Card(card_object) {
    ...
    // color method
    let _get_color = DA_Nav._color_hash.get_color;
    ...
    this._card_color  = _get_color(this._DA_class_id);
    function warning() {
        console.log(DA_Nav._color_hash._data);
    }
...
AIE_Card.prototype._eventBinding = function() {
    ...
    let frontImg = $(this._FrontGif).get(0);
