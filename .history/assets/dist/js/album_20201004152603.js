...
function setupInteraction() {
    ...
    $(window).resize(function() {
        ...
        nav = DA_Nav.NavQueue.init(".btn-primary-group", {
    ...
    let nav = DA_Nav.NavQueue.init(".btn-primary-group", {
        deltaY: 0.5*parseInt(document.querySelector("#card-display").getBoundingClientRect()["height"]),
        target: "#card-display"
    });

    document.querySelectorAll(".btn-secondary-group button, .btn-secondary-group li, .btn-secondary-group-sm button, .btn-secondary-group-sm li").forEach(node => {
...
DA_Nav.NavQueue = {
    ...
    init: function(element, option = {}) {
        ...
        nav.append = function(DA_Nav_object) {
            DA_Nav_object = DA_Nav_object instanceof DA_Nav ? DA_Nav_object : undefined;
            if(DA_Nav_object === undefined) return false;
            
            DA_Nav.NavQueue._object_list.push(DA_Nav_object);
        };

        nav.dispose = function() {
            nav._targetNode.removeEventListener("scroll", (e) => nav.scrollSpy(e));
            DA_Nav.NavQueue._content_list.length = 0;
            // nav.element = null;
            nav._deltaY = null;
            nav._target = null;
            nav._callback = null;
        ...
        nav.scrollSpy = function() {
            ...
            // console.log("Content List :", DA_Nav.NavQueue._content_list);
            DA_Nav.NavQueue._content_list.forEach((partNode, index, content_list) => {
                ...
                if(nav._targetNode.scrollTop >= partNode.offsetTop - nav._deltaY) {
                    // the last one
                    if(index === content_list.length-1) {
                        prev_passed = false;
                        activeNode = btnNode;
                        return true;
                    }
        ...
        nav._activateBtn = function(DA_Nav_node) {
            if(DA_Nav_node.classList.contains("active"))
                return false;

            DA_Nav_node.classList.add("active");
            return true;
        }

        // inactivate nav btn (node type)
        nav._inactivateBtn = function(DA_Nav_node) {
            if(!DA_Nav_node.classList.contains("active"))
                return false;

            DA_Nav_node.classList.remove("active");
            return true;
        }

        // choose visible & useful contents
        nav.refresh = function() {
            DA_Nav.NavQueue._content_list = [...nav._targetNode.querySelectorAll(`${nav._target} > div:not(.hidden):not(.search-fail)`)];
        }
        ...
        nav._targetNode.addEventListener("scroll", (e) => nav.scrollSpy(e));
...
DA_Nav.prototype.drawDATop = function() {
    ...
    let display_top = $("<div></div>").addClass("deck-reminder active-sticky")
                        .css({
                            "top": 0,
...
DA_Nav.prototype._interactionInit = function () {
    if(this._created) {
        ...
        this._display_deck = document.querySelector(`#${this._DA_joint_tag} > .card-deck`);
...
DA_Nav.prototype._getDisplayDeck = function() {
...
DA_Nav.prototype._getDisplayDesc = function() {
...
DA_Nav.prototype._topEventBinding = function () {
    ...
    //     // mark DA state & content of visible view
    //     if(parseInt(Math.round(window.scrollY)) >= parseInt(thisDA_Nav._sticky_top.offsetTop)-1.5*16) {
    //         if(!thisDA_Nav._inDisplay()){
    //             console.log("Forward!", thisDA_Nav._DA_nav_tag);
    ...
    //     } else {
    //         if(thisDA_Nav._inDisplay()) {
    //             console.log("Back!", thisDA_Nav._DA_nav_tag);
    ...
    let stickySpy = function() {
        ...
        if(parseInt(sticky_top.getBoundingClientRect()["y"]) > parseInt(container.getBoundingClientRect()["top"])) {
            // if sticky top has a changing top value in container view && higher than top Y -> not in sticky, shown (1)
            console.log("AAAAAAAAAAAAAA", thisDA_Nav._DA_nav_tag)
            

            if(thisDA_Nav._inSticky())
                thisDA_Nav._stickyToggle(false);

            if(sticky_top.style.opacity && parseInt(sticky_top.style.opacity) < 1){
                sticky_top.style.opacity = 1;
                sticky_top.style.backdropFilter = "none";
                sticky_top.style.backgroundColor = "transparent";
                sticky_top.style.borderBottom = "1px solid transparent";
                sticky_top.style.boxShadow = "none";
            }
        } else if(parseInt(sticky_top.getBoundingClientRect()["y"]) === parseInt(container.getBoundingClientRect()["top"])) {
            // if sticky top has a solid top value -> in sticky, shown (2)
            
            // if sticky top surpasses deltaY in container view but still has a solid top value -> not in sticky, hidden (2.1)
            if(thisDA_Nav._inSticky() && (parseInt(display_deck.getBoundingClientRect()["bottom"]) - parseInt(container.getBoundingClientRect()["top"]) <= deltaY)) {
                thisDA_Nav._stickyToggle(false);
                if(sticky_top.style.opacity && parseInt(sticky_top.style.opacity) > 0)
                $(sticky_top).fadeTo(400, 0);
            ...
            if(!thisDA_Nav._inSticky()) {
                console.log("BBBBBBBBBBBBBB", thisDA_Nav._DA_nav_tag);
                thisDA_Nav._stickyToggle(true);

                // $("#biology .deck-reminder").animate({
                ...
                // }, 360, () => {"good!"});
                sticky_top.style.opacity = 1;
                sticky_top.style.backdropFilter = "saturate(180%) blur(25px)";
                sticky_top.style.backgroundColor = "#FFFFFF72";
                sticky_top.style.borderBottom = "1px solid #00000016";
                sticky_top.style.boxShadow = "box-shadow": "0 1px 2px #DCDFEF32";
            }

        } else {
            // this sticky top has already been replaced by the next one -> not in sticky, hidden (3)
...
DA_Nav.prototype._scrollEventBinding = function () {
    ...
    thisDA_Nav._DA_btn.addEventListener("click", () => {
    ...
    window.onscroll = function() {
        ...
        if((parseInt(current_display_desc.getBoundingClientRect()["top"]) > parseInt(displayToTop))  || (parseInt(current_display_deck.getBoundingClientRect()["bottom"]) < (0.7*displayVisibleHeight) + parseInt(displayToTop))){
            ...
            if(thisStickyTop._shown === 0){
                ...
                console.log("show", current_DA_Nav._DA_nav_tag);
            }
        }
    }
}
...
DP_Tab.prototype._eventBinding = function() {
        ...
        li.addEventListener("click", function() {
            ...
            if(this.classList.contains("active")) {
                ...
                changedDeckList.forEach((part, index, arr) => {
                    ...
                    $(`#${part}`).fadeOut(470, () => {
                        if(index === arr.length - 1) DA_Nav.DP_fitting();
                    });
    ...
    thisDPTag._DP_primary_btn.addEventListener("click", function () {
        let targetCards, this_joint_DP_abr, changedDeckList;
        if(this.classList.contains("active")){
            this.classList.toggle("active", false);
