...
function createDP(DP_doc, classStr) {
    ...
    let btnClassStr = "btn text-left btn-block";
    if(classStr.indexOf("sm") > 0) {
        btnClassStr = "btn text-center";
    }
    ...
    $.getJSON(DP_doc, function(json) {
        ...
        $.each(json, function(i, item) {
            ...
            let DP_class_ul = $("<ul></ul>");
            let DP_class_tags = item.DA_sub_tag.split(",");
            DP_class_tags.forEach(function(DA_class_tag, index, arr) {
                DA_class_tag = $.trim(DA_class_tag);
                let DA_class_li = $("<li></li>")
                                    .addClass("active")
                                    .attr("id", AIE_Card.DP_abr(DA_class_tag))
                                    .text(DA_class_tag);
                DP_class_ul.append(DA_class_li);
            });

            DP_Group.append(
                $("<button></button>")
                .addClass(btnClassStr)
                .text(DP_tag)
                .prepend($("<span></span>"))
            ).append(
                DP_class_ul.addClass("btn-sub-list")
            );
        });
...
function DP_Tab(DP_object) {
    ...
    let DP_sub_tag = DP_object._DP_sub_tag.split(",");
    DP_sub_tag.forEach(tag => DP_sub_tag.push(tag.trim()));
    this._DP_sub_tag = DP_sub_tag; // array
}

DP_Tab.prototype._drawDP = function(btnClassStr) {
    btnClassStr = btnClassStr || "btn text-left btn-block";
    let DP_primary_button = $("<button></button>").addClass(classStr);
}
