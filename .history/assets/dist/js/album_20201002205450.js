...
function createDA(DA_doc, classStr) {
    ...
    // stickyTopElem.fadeTo(0, 0);

    // create NS part
    let DA_Group = $("<div></div>").addClass(classStr).attr("id", "display-scroll");
...
DA_Nav.prototype.DACreatingComplete = function(StickyTop_object) {
    // ......
    if(this._created <= 0 && StickyTop_object !==undefined) {
        this._created = new Date().getTime();
        this._sticky_top_object = StickyTop_object;
...
DA_Nav.prototype._interactionInit = function () {
    if(this._created) {
        ...
        // record in/out of visible view
        this._in_display = false;

        // record show/hide situ of sticky top
...
DA_Nav.prototype._recordPrevDA_Nav = function(DA_Nav_object) {
    ...
    DA_Nav_object = DA_Nav_object instanceof DA_Nav ? DA_Nav_object : undefined;

    this._prev_DA_Nav = DA_Nav_object;
    return true;
}

DA_Nav.prototype._getPrevDA_Nav = function() {
    if(!this.hasOwnProperty("_prev_DA_Nav")) {
        console.log("Either sticky top or DA button has been deployed yet.");
        return undefined;
    }
...
DA_Nav.prototype._getStickyTop = function() {
...
DA_Nav.prototype._displayToggle = function(option, callback) {
    option = option || undefined;
    callback = callback || undefined;
    
    if(!this.hasOwnProperty("_in_display")) {
        console.log("Either sticky top or DA button has been deployed yet.");
        return false;
    } else if(option === true) {
        this._in_display = true;
    } else if(option === false) {
        this._in_display = false;
    } else {
        this._in_display = !this._in_display;
    }
...
DA_Nav.prototype._inDisplay = function() {
    return this._in_display;
}
...
DA_Nav.prototype._inSticky = function() {
...
DA_Nav.prototype._topEventBinding = function () {
    ...
    let thisDA_Nav = this;
    let stickyTopElem = this._sticky_top_object.getStickyTop();
    let cardDisplay = document.getElementById(`card-display`);
    let displayToTop = function() {return cardDisplay.getBoundingClientRect().y + cardDisplay.ownerDocument.defaultView.pageYOffset};

    let stickyOnAnimation = function(sticky_top) {
        // sticky_top.classList.toggle("active", true);
        $(sticky_top).fadeTo("normal", 0).fadeTo("normal", 1);
    };
    let stickyOffAnimation = function(sticky_top) {
        // sticky_top.classList.toggle("active", false);
        $(sticky_top).fadeTo("normal", 0).fadeTo("normal", 1);
    }
    ...
    $(window).scroll(function(){
        ...
        let prev_DA_Nav;
        let current_displayToTop = displayToTop();
        let displayVisibleHeight = parseInt(window.innerHeight) - parseInt(current_displayToTop);

        let scrollToStickyTop = parseInt(thisDA_Nav._sticky_top.offsetTop) - parseInt(Math.round(window.scrollY));
        
        // mark DA state & content of sticky top
        if(parseInt(Math.round(window.scrollY)) > parseInt(thisDA_Nav._sticky_top.offsetTop) && !thisDA_Nav._inDisplay()) {
            console.log("yes!", thisDA_Nav._DA_nav_tag);
            [ , prev_DA_Nav] = thisDA_Nav._sticky_top_object.showDisplay(thisDA_Nav);
            thisDA_Nav._displayToggle(true);
            thisDA_Nav._recordPrevDA_Nav(prev_DA_Nav);
        } else if(parseInt(Math.round(window.scrollY)) <= parseInt(thisDA_Nav._sticky_top.offsetTop) && thisDA_Nav._inDisplay()) {
            console.log("No!", thisDA_Nav._DA_nav_tag);
            prev_DA_Nav = thisDA_Nav._getPrevDA_Nav();
            thisDA_Nav._displayToggle(false);
            thisDA_Nav._sticky_top_object.showDisplay(prev_DA_Nav);
        }

        // if((parseInt(thisDA_Nav._sticky_top.getBoundingClientRect().bottom) <= current_displayToTop)) {
        //     console.log("0")
        //     console.log(thisDA_Nav._DA_nav_tag);
        //     stickyOnAnimation(stickyTopElem);
        // } else if(scrollToStickyTop <= 0.5*displayVisibleHeight && scrollToStickyTop > 0) {
        //     console.log("1")
        //     console.log(thisDA_Nav._DA_nav_tag);
        //     stickyOffAnimation(stickyTopElem);
        // }

        console.log((parseInt(Math.round(window.scrollY)) < parseInt(thisDA_Nav._display_desc.offsetTop)) && (parseInt(Math.round(window.scrollY)) > parseInt(thisDA_Nav._sticky_top.offsetTop) - 0.33*displayVisibleHeight), thisDA_Nav._DA_nav_tag);
        // if(thisDA_Nav._inSticky() && (parseInt(Math.round(window.scrollY)) < parseInt(thisDA_Nav._display_desc.offsetTop)) && (parseInt(Math.round(window.scrollY)) > parseInt(thisDA_Nav._sticky_top.offsetTop) - 0.33*displayVisibleHeight)) {
        //     // thisDA_Nav._stickyToggle(false, () => thisDA_Nav._sticky_top_object.fadeTo("normal", 0));
        //     thisDA_Nav._stickyToggle(false);
        //     console.log("out", thisDA_Nav._DA_nav_tag);
        //     // console.log(window.scrollY, thisDA_Nav._display_desc.offsetTop);
        //     // console.log(window.scrollY, parseInt(thisDA_Nav._sticky_top.offsetTop) - 0.33*displayVisibleHeight);
        // } else if(!thisDA_Nav._inSticky() && thisDA_Nav._inDisplay()) {
        //     thisDA_Nav._stickyToggle(true);
        //     // thisDA_Nav._stickyToggle(true, () => thisDA_Nav._sticky_top_object.fadeTo("normal", 1));
        //     console.log("in", thisDA_Nav._DA_nav_tag);
        // //     console.log(window.scrollY, thisDA_Nav._display_desc.offsetTop);
        // //     console.log(window.scrollY, parseInt(thisDA_Nav._sticky_top.offsetTop) - 0.33*displayVisibleHeight);
        // }

        // if((parseInt(Math.round(window.scrollY)) < parseInt(thisDA_Nav._display_desc.offsetTop)) && (parseInt(Math.round(window.scrollY)) > parseInt(thisDA_Nav._sticky_top.offsetTop) - 0.33*displayVisibleHeight)) {
        //     if(thisDA_Nav._inSticky()) {
        //         console.log("out", thisDA_Nav._DA_nav_tag);
        //         thisDA_Nav._stickyToggle(false);
        //     }
        // } else if(thisDA_Nav._inDisplay() && !thisDA_Nav._inSticky()) {
        //     console.log("in", thisDA_Nav._DA_nav_tag);
        //     thisDA_Nav._stickyToggle(true);
        // }
    });
...
function StickyTop(DA_Nav_object) {
    
    this._DA_Nav_arr = DA_Nav_object instanceof DA_Nav ? [DA_Nav_object, undefined] : [undefined, undefined];
    ...
    let sticky_top = DA_Nav_object instanceof DA_Nav ? createStickyTop(sticky_top_object._DA_Nav_arr[0]._getStickyTop()) : document.createElement("div");
    sticky_top.classList.add("deck-reminder", "active-sticky");
...
StickyTop.prototype.appendToDisplay = function(container, top) {
    ...
    this._sticky_top.style.cssText = ` position: fixed; width: ${width}px; z-index: 500; top: ${top}px; opacity: 0;`;
...
StickyTop.prototype.showDisplay = function(DA_Nav_object, sticky_top_container) {
    ...
    // this._DA_sticky_top_arr[1] = this._DA_sticky_top_arr[0];
    // this._DA_sticky_top_arr[0] = DA_sticky_top_elem;
    // return this._DA_sticky_top_arr;
}
