...
function setupInteraction() {
    ...
    let navInit = function(){
        let nav;
        if(document.querySelector(".btn-primary-group") !== null || document.querySelector(".btn-primary-group").length > 0)
        ...
        else if(document.querySelector(".btn-primary-group-sm") !== null || document.querySelector(".btn-primary-group-sm").length > 0)
            nav = DA_Nav.NavQueue.init(".btn-primary-group-sm", {
                deltaY: 0.25*parseInt(document.querySelector("#card-display").getBoundingClientRect()["height"]),
                target: "#card-display"
            });

        document.querySelectorAll(".btn-secondary-group button, .btn-secondary-group li").forEach(node => {
...
DA_Nav.NavQueue = {
    ...
    init: function(element, option = {}) {
        ...
        nav.dispose = function() {
            nav._targetNode.removeEventListener("scroll", nav.scrollSpy);
        ...
        nav.scrollSpy = function() {
            ...
            DA_Nav.NavQueue._content_list.forEach((partNode, index, content_list) => {
                ...
                if(nav._targetNode.scrollTop >= partNode.offsetTop - nav._deltaY) {
                    ...
                    prev_passed = true;
                    activeNode = btnNode;
                    console.log("AAA", Id + " " + prev_passed);
                } else if(prev_passed) {
                    prev_passed = false;
                    console.log("BBB", Id + " " + prev_passed);
                    return true;
                }
            });
            console.log("IDID", activeNode);
