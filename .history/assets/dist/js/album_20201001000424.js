...
DA_Nav.prototype._recordPrevDA = function(DA_Nav_Object) {
    ...
    if((DA_Nav_Object !== undefined) && (DA_Nav_Object instanceof DA_Nav)) {
        console.log(DA_Nav_Object, "is not a DA_Nav object.");
        return false;
    }

    this._prev_DA = DA_Nav_Object;
    return true;
}

DA_Nav.prototype._getPrevDA = function() {
    if(!this.hasOwnProperty("_prev_DA")) {
        console.log("Either sticky top or DA button has been deployed yet.");
        return undefined;
    }

    return this._prev_DA;
}
...
DA_Nav.prototype._topEventBinding = function () {
    ...
    $(window).scroll(function(){
        ...
        if(parseInt(Math.round(window.scrollY)) > parseInt(thisDA_Nav._sticky_top.offsetTop) && !thisDA_Nav._isSticky()) {
            ...
            let prev_DA_Nav = thisDA_Nav._getPrevDA();
            let prev_sticky_top = prev_DA_Nav._getStickyTop();
...
function StickyTop(DA_sticky_top) {
    ...
    this._DA_sticky_top_arr = [undefined, DA_sticky_top] || [undefined];
    ...
    this._sticky_top = createStickyTop(this._DA_sticky_top_arr[1]);
}
