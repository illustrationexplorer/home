...
function reminderSpy() {
    ...
    allReminders.some(function(sticky, index, nodeList) {
        ...
        if(current_active_sticky.getBoundingClientRect().bottom <= current_active_sticky.nextElementSibling.getBoundingClientRect().top) {
            current_active_sticky.classList.remove("active-sticky");
            $($(current_active_sticky).find(".display-desc").get(0)).slideDown(240);
            return false;
        }
