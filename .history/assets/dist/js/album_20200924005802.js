...
function reminderSpy() {
    ...
    let current_active_sticky =document.querySelector(".active-sticky");
    document.querySelectorAll(".deck-reminder").forEach(function(sticky, index, nodeList) {
        let reminderToHeader = parseInt(Math.round(sticky.getBoundingClientRect().top)) - nav.offsetHeight;
        if(current_active_sticky && reminderToHeader < (displayHeight / 2)) {
            console.log(current_active_sticky);
            current_active_sticky.classList.remove("active-sticky");
            // current_active_sticky.querySelector(".display-desc")
            return ;
        }

        if(reminderToHeader <= nav.offsetHeight) {
