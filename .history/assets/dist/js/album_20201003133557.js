...
DP_Tab.prototype._eventBinding = function() {
    ...
    let deckChecking = function(option) {
        ...
        document.querySelectorAll(".card-deck").forEach(deck => {
            ...
            if((!option) && (getComputedStyle(displayPart, null)["display"] === "none" || deck.querySelectorAll(".screened-out").length > 0))
                return ;
            else if(option && (getComputedStyle(displayPart, null)["display"] !== "none" || deck.querySelectorAll(".screened-out").length <= 0))
                return ;
    ...
    thisDPTag._DP_sub_li.forEach(li => {
        ...
        li.addEventListener("click", function() {
            ...
            if(this.classList.contains("active")) {
                ...
                console.log("to-fade", targetCards);
                targetCards.forEach(node => {
                    ...
                    }).fadeTo(1, 1, () => console.log("should be hidden all"));
                });

                // 把上面的函数分开为两部分，deck判断作为夹心
                // changedDeckList = deckChecking(false); // new empty list
                // changedDeckList.forEach(displayPart => $("#" + displayPart).fadeOut("fast"));
            } else {
                this.classList.toggle("active", true);
                targetCards = document.querySelectorAll(`.${this_DP_abr}.screened-out`);
                console.log("to-show", targetCards);
                targetCards.forEach(node => node.classList.remove("screened-out"));
                $(targetCards).show(1, () => {console.log("should be showed all")}).fadeTo(600, 1, () => console.log("should be visible all"));

                // changedDeckList = deckChecking(true); // new non empty list
                // changedDeckList.forEach(displayPart => $("#" + displayPart).fadeIn("fast"));
            }

            // console.log("changing: ", changedDeckList);
        });
    });
