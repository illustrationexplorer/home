    ...
    thisDPTag._DP_sub_li.forEach(li => {
        ...
        li.addEventListener("click", function() {
            if(this.classList.contains("active")) {
                ...
                $("#card-display").find(`.${this_DP_abr}:visible`).fadeTo(360, 0).animate({"transform": "scaleX(0)"}, 400).hide(1);
            } else {
                this.classList.toggle("active", true);
                $("#card-display").find(`.${this_DP_abr}:hidden`).show(1).animate({"transform": "scaleX(1)"}, 400).fadeTo(360, 1);
            }
        });
    });
}
