...
function reminderSpy() {
    ...
    allReminders.some(function(sticky, index, nodeList) {
        ...
        if(current_active_sticky && (reminderToHeader > current_active_sticky.offsetHeight)) {
            console.log("A");
            sticky.classList.remove("active-sticky");
            $($(sticky).find(".display-desc").get(0)).slideDown();
            return false;
        }

        if(Math.abs(reminderToHeader) < 5 || Math.abs(reminderToHeader) > 1) {
            console.log("B");
