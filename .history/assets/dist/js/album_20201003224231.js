...
DA_Nav.NavQueue = {
    ...
    init: function(element, option = {}) {
        ...
        element = element || ".list-group"; // nav btn ul

        nav._deltaY = option["deltaY"] || 0;
        nav._target = option["target"] || "body";
        nav._callback = option["callback"] || function() {return ;};

        DA_Nav.NavQueue._btn_list = [...document.querySelectorAll(`${element} a`)];
        DA_Nav.NavQueue._btn_list = [...document.querySelectorAll(`${nav._target} > div:not(.search-fail)`)];
        ...
        nav._activateBtn = function(DA_Nav_node) {
            if(DA_Nav_node === null || DA_Nav_node.classList.contains("active") || DA_Nav_node.classList.contains("disabled"))
                return false;
