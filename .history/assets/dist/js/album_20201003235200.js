    ...
    init: function(element, option = {}) {
        ...
        nav.scrollSpy = function() {
            DA_Nav.NavQueue._content_list.forEach((partNode, index, content_list) => {
                ...
                if(nav._targetNode.scrollTop < partNode.offsetTop - nav._deltaY && index > 0){
                    // too early to active
                    if(btnNode.classList.contains("active"))
                        nav._inactivateBtn(btnNode);

                    // last one should be activated
                    if(prev_passed && !activeNode.classList.contains("active")){
        ...
        nav.refresh = function() {
            ...
            DA_Nav.NavQueue._btn_list = [...nav._targetNode.querySelectorAll(`${nav._target} > div:not(.hidden):not(.search-fail)`)];
        }
