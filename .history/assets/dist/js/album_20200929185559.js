...
DA_Nav.prototype._topEventBinding = function () {
    ...
    let thisDA_Nav = this;
    let desc_rect = thisDA_Nav._display_desc.getBoundingClientRect();
    ...
    let marginChange;
    let thisDeck = thisDA_Nav._sticky_top.nextElementSibling;
    let thisDeck_paddingTop = parseInt(window.getComputedStyle(thisDeck, null)["paddingTop"]);

    $(window).scroll(function(){
        // mark DA state
        if(parseInt(Math.round(window.scrollY)) === thisDA_Nav._sticky_top.offsetTop && !thisDA_Nav._isSticky()) {
            marginChange = desc_rect.height + parseInt(getComputedStyle(thisDA_Nav._display_desc, null)["marginBottom"]);
            thisDeck.style.paddingTop = `${thisDeck_paddingTop + marginChange}px`;
            ...
            console.log("Nonono! ", thisDA_Nav._DA_nav_tag);
            thisDA_Nav._stickyToggle(false, stickyOffAnimation);
            thisDeck.style.paddingTop = `${thisDeck_paddingTop}px`;
        }
    });
}
