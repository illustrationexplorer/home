...
let color_hash = {
    private: function() {
        let hash = { 500: "#666666" };
        let setHash = function(key, value) {
            hash[key] = value;
            return true;
        }
        let getHash = function(key) {
            if(hash.hasOwnProperty(key))
                return hash[key];
            else
                return undefined;
        }
        return {setHash, getHash};
    },
    data: function() {
        return 
    }
...
function DA_Nav(DA_object) {
    ...
    // color setting
    let _color_hash = { 500: "#EBEDF6" };
    let _DA_nav_color = DA_object["DA_nav_color"] || "#EBEDF6";
    let set_color = function(key_id, color_str) {
        key_id = key_id || 500;
        color_str = color_str || "";
        if(color_str) {
        ...
        _color_hash[key_id] = color_str;
        return true;
    }

    this.get_color = function(key_id) {
        key_id = key_id || 500;
        if(_color_hash.hasOwnProperty(key_id)) 
            return _color_hash[key_id];
        else
            return undefined;
    }
    ...
    this._DA_sub_arr = DA_object["DA_class_object"];

    set_color(this._DA_id, _DA_nav_color);
}
...
DA_Nav.prototype.drawDA = function() {

}

DA_Nav.prototype.drawTop = function() {
