...
DP_Tab.prototype._eventBinding = function() {
    ...
    // bind hide/visible event to sub button
    thisDPTag._DP_sub_li.forEach(li => {
        let this_DP_abr = li.getAttribute("id");
        li.addEventListener("click", function() {
            if(this.classList.contains("active")) {
                this.classList.toggle("active", false);
                let targetCards = document.querySelectorAll(`.${this_DP_abr}:not(.hidden)`);
                targetCards.forEach(node => node.classList.add("hidden"));
                $(targetCards).fadeTo(400, 0).hide(1);
            } else {
                this.classList.toggle("active", true);
                let targetCards = document.querySelectorAll(`.${this_DP_abr}.hidden`);
                targetCards.forEach(node => node.classList.remove("hidden"));
