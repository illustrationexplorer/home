...
DA_Nav.prototype._getPrevDA_Nav = function() {
    if(!this.hasOwnProperty("_prev_sticky_top")) {
        console.log("Either sticky top or DA button has been deployed yet.");
        return undefined;
    }
...
DA_Nav.prototype._topEventBinding = function () {
    ...
    $(window).scroll(function(){
        ...
        if(parseInt(Math.round(window.scrollY)) > parseInt(thisDA_Nav._sticky_top.offsetTop) && !thisDA_Nav._isSticky()) {
            // console.log("yes!", thisDA_Nav._DA_nav_tag);
            [ , prev_DA_Nav] = thisDA_Nav._sticky_top_object.showDisplay(thisDA_Nav._sticky_top);
            prev_DA_Nav._stickyToggle(false);
            thisDA_Nav._stickyToggle(true);
            thisDA_Nav._recordPrevDA_Nav(prev_DA_Nav);
        } else if(parseInt(Math.round(window.scrollY)) <= parseInt(thisDA_Nav._sticky_top.offsetTop) && thisDA_Nav._isSticky()) {
            // console.log("No!", thisDA_Nav._DA_nav_tag);
            prev_DA_Nav = thisDA_Nav._getPrevDA_Nav();
            thisDA_Nav._stickyToggle(false);
            prev_DA_Nav._stickyToggle(true);
            thisDA_Nav._sticky_top_object.showDisplay(prev_DA_Nav);
        }

        // if((parseInt(thisDA_Nav._sticky_top.getBoundingClientRect().bottom) <= current_displayToTop) && !stickyTopElem.classList.contains("active")) {
        //     console.log("0")
        //     console.log(thisDA_Nav._DA_nav_tag);
        //     console.log(parseInt(thisDA_Nav._sticky_top.getBoundingClientRect().bottom) - current_displayToTop)
        //     stickyOnAnimation(stickyTopElem);
        // } else if(parseInt(Math.round(window.scrollY)) > parseInt(thisDA_Nav._sticky_top.offsetTop) - 0.5*displayVisibleHeight) {
            // console.log("1")
            // console.log(thisDA_Nav._DA_nav_tag);
...
StickyTop.prototype.appendToDisplay = function(container, top) {
    ...
    top = top || container.getBoundingClientRect()["height"] + container.ownerDocument.defaultView.pageYOffset;
