...
function createDA(DA_doc, classStr) {
    ...
    let DA_Nav_list = []; // for DA_Nav objects
    let displayPart_list = []; // for display parts
    ...
    $.getJSON(DA_doc, function (json) {
        $.each(json, function (i, item){
            ...
            // init for scroll spy
            DA_Nav_list.push(DA_single);
            displayPart_list.push(currentDisplayPart);
        });
    });
}
    ...
    init: function(element, option = {}) {
        ...
        nav.scrollSpy = function() {
            DA_Nav.NavQueue._content_list.forEach((partNode, index, content_list) => {
                ...
                // let btnNode = DA_Nav.NavQueue._btn_list[index];

                let prevNode;
                let prev_passed = false;
                if(nav._targetNode.scrollTop < partNode.offsetTop - nav._deltaY){
                    // the last passed one should be activated
                    if(prev_passed && !prevNode.classList.contains("active")){
                        nav._activateBtn(prevNode);
                        prev_passed = false;
                        return false;
                    }
                    
                    // too early to active
                    if(!prev_passed && btnNode.classList.contains("active"))
                        nav._inactivateBtn(btnNode);

                    if(!prev_passed && prevNode.classList.contains("active"))
                        nav._inactivateBtn(prevNode);

                } else {
                    // too late to be activated
                    if(prev_passed && prevNode.classList.contains("active"))
                        nav._inactivateBtn(prevNode);
        ...
        nav.refresh = function() {
            // DA_Nav.NavQueue._btn_list = [...document.querySelectorAll(`${element} a:not(.disabled)`)];
